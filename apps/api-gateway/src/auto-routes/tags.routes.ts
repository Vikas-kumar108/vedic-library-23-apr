...
import { TagSchema } from '@dharma/contracts';
import { z } from 'zod';

/**
 * 🛰️ Auto-generated CRUD for tags
 */
export default async function tagsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async (request, reply) => {
    ...
    return data.map((item: any) => TagSchema.parse(item));
  });

  // 2. GET BY ID
  fastify.get('/:id', async (request: any, reply) => {
    const { id } = request.params;
    const item = await prisma.tags.findUnique({
      where: { id }
    });

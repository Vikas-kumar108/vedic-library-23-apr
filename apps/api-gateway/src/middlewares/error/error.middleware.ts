 ...
 */
export const errorMiddleware = (error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
 ...
 */
export const transactionMiddleware = async (request: FastifyRequest) => {
  // Logic: Signal the repository layer to start a Prisma transaction
  ;(request as any).useTransaction = true
}

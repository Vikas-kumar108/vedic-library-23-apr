...
export async function POST(
  ...
  try {
    ...
    // Roles that can confirm donations: director, super_admin, ca
    const confirmRoles = ["director", "super_admin", "ca", "admin"]
    if (!confirmRoles.includes(session.user.role)) {
      return NextResponse.json({ error: "You don't have permission to confirm donations" }, { status: 403 })
    }

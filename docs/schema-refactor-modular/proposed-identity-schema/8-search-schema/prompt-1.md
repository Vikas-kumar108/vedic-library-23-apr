You are acting as a senior system architect working within this codebase.

Context
This is a monorepo application using:
React / Next.js (frontend)
Node.js (backend APIs)
PostgreSQL (database)
Current state:
Entire database exists in a single public schema (~4,000 lines)
APIs, business logic, and frontend are already implemented
Data is minimal, but existing logic must remain stable
Refactoring goal:
Move from public → domain-driven schemas
Current focus: Search / Advanced AI Domain (search)
Scope of Search / Advanced AI Domain

This domain includes:

Search indexing (full-text, structured, hybrid)
Query logs and analytics
Relevance scoring and ranking data
AI-assisted search (semantic search, embeddings, vector storage)
Content indexing across domains (knowledge, education, community, etc.)
Recommendations / personalization signals (if tied to search)
Task

Design a production-grade PostgreSQL search schema.

Requirements
1. Schema Design
Define all tables with:
Clear responsibilities
Proper normalization (where applicable)
Logical grouping (indexing, queries, embeddings, analytics)
Ensure:
Support for high-performance querying
Compatibility with vector extensions (e.g., embeddings)
Scalability for large datasets
2. Enum Strategy
Define all required enums, such as:
Query type
Result type
Index type
Embedding source/type
Search status
Specify:
Enum values
Where and how they are used
3. Relationships
Define:
Indexed entities ↔ source domains (knowledge, education, etc.)
Queries ↔ results ↔ engagement
Embeddings ↔ content
Include foreign keys and constraints
4. Mapping from Existing Schema
Based on current public schema:
Identify relevant tables (if any)
Suggest:
Move / Split / Merge / Remove
5. Boundaries
Clearly define:
What belongs in search
What should NOT be included (e.g., actual content storage, user auth)
6. Migration Awareness (Design-Level Only)
Ensure the design supports:
Smooth migration from public
Minimal disruption to existing APIs and frontend
Constraints
Do NOT modify application code
Do NOT generate migration scripts
Do NOT refactor APIs

Focus only on:

Schema design
Structure
Clarity
Output Format

Provide:

Schema Overview
Table-by-Table Definitions
Enum Definitions
Relationship Mapping
Mapping from Current Schema
Key Design Decisions
Goal

Produce a clean, scalable, and AI-ready search schema that supports:

Advanced querying and indexing
Semantic search and embeddings
Analytics and relevance tuning
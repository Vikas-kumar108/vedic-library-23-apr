...
const DEV_TO_IAST_MARKS = {
    'ा': 'ā', 'ि': 'i', 'ी': 'ī', 'ु': 'u', 'ू': 'ū', 'ृ': 'ṛ', 'ॄ': 'ṝ', 'ॢ': 'ḷ', 'ॣ': 'ḹ',
    'े': 'e', 'ै': 'ai', 'ो': 'o', 'ौ': 'au', '्': '',
};
const DEV_TO_IAST_VOWELS = {
    'अ': 'a', 'आ': 'ā', 'इ': 'i', 'ई': 'ई', 'उ': 'u', 'ऊ': 'ū', 'ऋ': 'ṛ', 'ॠ': 'ṝ', 'ऌ': 'ḷ', 'ॡ': 'ḹ',
    'ए': 'e', 'ऐ': 'ai', 'ओ': 'o', 'औ': 'au'
};
const DEV_TO_IAST_CONSONANTS = {
 ...
 */
function transliterate(text, from, to) {
    if (!text || from === to)
        return text;
...
function iastToDevanagari(text) {
    let result = '';
    let i = 0;
    const isVowel = (char) => 'aāiīuūṛṝḷḹeaiou'.includes(char);
    const isModifier = (char) => 'ṃḥ'.includes(char);
    while (i < text.length) {
        let match = '';
        ...
        if (!match) {
            ...
            if (IAST_TO_DEV[one] !== undefined || DEV_VOWELS[one])
                match = one;
        }
        if (match) {
